...
class Informe2Export implements FromView
{
  ...
  public function view(): View
  {
    ...
    foreach ($notaFinalEstudianteActivos as $item) {
        //dd("Estamos en reporte final",$item);
    ...
    $ordenDeseado = ['PRIMER PERIODO', 'SEGUNDO PERIODO', 'TERCER PERIODO', 'CIENCIAS NATURALES', 'RELIGION - ETICA Y VALORES','SOCIALES','INFORMATICA','EDUCACION FISICA','ARTISTICA','COMPORTAMIENTO'];
        ...
        $agrupado = $notas->groupBy('nom_estudiante')->map(function ($items, $estudiante) {
                ...
                $count = 0;
                    foreach ($items as $item) {
                        $sumaMateria = floatval(isset($item['nota_periodo_uno']) ? $item['nota_periodo_uno'] : 0) +
                        ...
                        $promedio = $sumaMateria / 3;
                        $promFinal = round($promedio,2);
                        $fila[$item['desc_materia']] = $promFinal;
                        $suma += $promFinal;
                        $count++;
                    }
