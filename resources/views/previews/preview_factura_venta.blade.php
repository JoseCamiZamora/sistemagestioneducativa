<div class="container">
    <div class="row">
            ...
            <div class="col-md-12 text-right">
                ...
                @if($facturaVenta->facturado_e=="SI")
                      <div class="alert alert-success" role="alert">
                         Esta factura Ya se encuentra enviada y aprobada por La DIAN
                     </div>

                @else

                    <a href="javascript:void(0);" onclick="Facturar_dian({{$facturaVenta->id}});" class="mb-2 btn  btn-success mr-1">
                ...
                <div class="card-body p-0">
                    <div class="row ">
                        ...
                        <div class="col-md-6 text-right">
                            <p class="font-weight-bold mb-4 p-1 mr-5">Factura de venta código:  {{ $facturaVenta->codigo_factura  ?? 0 }}</p>
                        </div>
                    </div>
                    <hr class="my-0">
                    <div class="row pb-5 p-5">
                        <div class="col-md-4">
                          <p class="font-weight-bold mb-4">Factura código:  {{ $facturaVenta->codigo_factura  ?? 0 }}</p>
                             <p class="mb-1"><span class="text-muted">Fecha Expedición: </span>  {{ $facturaVenta->fecha_expedicion_fac ?? 0 }}</p>
                        </div>
                        <div class="col-md-4">
                        ...
                        <div class="col-md-4 text-right">
                            ...
                            <p class="mb-1"><span class="text-muted">Nombre: </span>  {{ $facturaVenta->nombre_cliente ?? 0 }}</p>
                            <p class="mb-1"><span class="text-muted">Identificación: </span>{{ $facturaVenta->identificacion_cliente ?? 0 }}</p>
                            <p class="mb-1"><span class="text-muted">Dirección: </span>{{ $facturaVenta->direccion_cliente ?? 0 }}</p>
                            <p class="mb-1"><span class="text-muted">Correo electronico: </span>{{ $facturaVenta->email ?? 0 }}</p>
                        </div>
                    </div>
                    <div class="row p-3">
                    	<div class="col-md-12">
                            ...
                            <table class="table">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>{{  $insSEL->valor_unitario }}</td>
                                        <td>{{  $insSEL->valor_bruto }}</td>
                                        <td>{{  $insSEL->iva}}</td>
                    ...
                    <div class="d-flex flex-row-reverse text-white p-4" style="background-color: #c0c8d3 !important;">
                        <div class="py-3 px-5 text-right">
                            ...
                            <div class="h4 font-weight-bold">${{  number_format( $facturaVenta->total , 2,',' , '.' )  ?? 0 }}</div>
                        </div>

                        <div class="py-3 px-5 text-right">
                            <div class="mb-2 font-weight-bold">Iva</div>
                            <div class="h4 font-weight-bold">${{ number_format(  $facturaVenta->total_iva , 2,',' , '.' )  ?? 0 }}</div>
                        </div>

                        <div class="py-3 px-5 text-right">
                            <div class="mb-2 font-weight-bold">Subtotal</div>
                            <div class="h4 font-weight-bold">${{ number_format(  $facturaVenta->total_vbruto , 2,',' , '.' ) ?? 0 }}</div>
                        </div>
                    </div>
                </div>
            </div>
